...
fn date_to_newyear(
    ...
    month: u32,
    day: u32,
    date2newyear: &HashMap<u32, (u32, u32)>,
) -> u32 {
    let mut newyear_year = year;
    let mut newyear_month;
    let mut newyear_day;
    ...
    if month > newyear_month || (month == newyear_month && day > newyear_day) {
        (newyear_month, newyear_day) = *date2newyear
            .get(&(year + 1))
            .expect("date_to_newyear error");
        newyear_year = year + 1;
    }
...
fn is_holiday(year: u32, month: u32, day: u32, date2newyear: &HashMap<u32, (u32, u32)>) -> bool {
    ...
    // 春节
    let (mut newyear_month, mut newyear_day) =
        *date2newyear.get(&year).expect("date_to_newyear error");
    if newyear_day != 1 {
...
pub fn time_info(time: &str) -> String {
    ...
    format!(
        "{},{},{},{},{},{}",
        week_count, week_day, days_0, days_1, days_2, days_3
    )
}